...
My_Vector<T>::My_Vector(initializer_list<U> arr)
{
    ...
    std::ranges::copy(arr.begin(), arr.end(), this->begin());
}
...
{
    int size = std::ranges::distance(begin, end);
    this->check_vec_size(size, __LINE__);
    this->count_axis = size;
    this->mem_alloc(this->count_axis, __LINE__);
...
My_Vector<T>::My_Vector(const My_Vector<T> &other)
{
    ...
    this->mem_alloc(this->count_axis, __LINE__);
    std::ranges::copy(other.cbegin(), other.cend(), this->begin());
}
...
template<std::ranges::input_range Range>
    requires requires(typename Range::value_type T1)  {{ T1 }  -> convertible_to<T>;  }
My_Vector<T>::My_Vector(Range&& range)
{
    this->count_axis = std::ranges::distance(range);
    mem_alloc(this->count_axis, __LINE__);
...
{
    bool rc = this->count_axis == (int)other.size();
    if (rc)
    {
        My_CIterator<T> iter = this->begin();
        for (auto iter1 = other.begin(); iter1 != other.end() && rc; iter1++)
        {
            if (fabs(*iter - *iter1) > EPS)
                rc = false;
            iter++;
        }
    }
...
template<std::ranges::input_range Range>
    requires requires(typename Range::value_type T1)  {{ T1 }  -> convertible_to<T>;  }
bool My_Vector<T>::is_equal(Range&& range) const
{
    My_Vector<T> other(range);
    return *this == other;
}
...
template<std::ranges::input_range Range>
    requires requires(typename Range::value_type T1)  {{ T1 }  -> convertible_to<T>;  }
My_Vector<T>& My_Vector<T>::operator = (Range&& range)
{
    this->count_axis = std::ranges::distance(range);
    mem_alloc(this->count_axis, __LINE__);
    std::ranges::copy(range, this->begin());
...
ostream& operator << (ostream &output, My_Vector<T> const &vector)
{
    ...
    {
        output << "( ";
        std::ranges::copy(vector.begin(), vector.end(), std::ostream_iterator<T>(output, " "));
        output << ")";
    }
    return output;
}
